...
# when docker run loads the `.env.opt` file, it keeps the quotes around the password
# which causes the password to be incorrect.
# To test that try running `docker run --rm --env-file .env.opt busybox /bin/sh -c 'echo $NODE_ECDSA_KEY_PASSWORD'`
...
optIn() {
  ...
  public.ecr.aws/altlayer/mach-operator-tool:v0.2.3 \
